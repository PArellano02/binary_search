    ...
    def binary(xs, left, right):
        ...
        if xs[mid] > 0:
            if mid == 0 or xs[mid - 1] <= 0:
                ...
                return binary(xs, left, mid - 1)
        else:
            return binary(xs, mid + 1, right)
    return binary(xs, 0, len(xs) - 1)
...
def count_repeats(xs, x):
    ...
    def get_first(xs, left, right, x):
        if left > right:
            return None
        mid = (left + right) // 2
        if xs[mid] == x:
            if mid == 0 or xs[mid - 1] >= x + 1:
                return mid
            else:
                return get_first(xs, left, mid - 1, x)
        elif xs[mid] > x:
            return get_first(xs, mid + 1, right, x)
        else:
            return get_first(xs, left, mid - 1, x)

    def get_second(xs, left, right, x):
        if left > right:
            return None
        mid = (left + right) // 2
        if xs[mid] == x:
            if mid == len(xs) - 1 or xs[mid + 1] < x:
                ...
                return get_second(xs, mid + 1, right, x)
        elif xs[mid] < x:
            return get_second(xs, left, mid - 1, x)
        else:
            return get_second(xs, mid + 1, right, x)

    try:
        second = get_second(xs, 0, len(xs) - 1, x)
        first = get_first(xs, 0, len(xs) - 1, x)
        return second - first
    except TypeError:
        return 0
